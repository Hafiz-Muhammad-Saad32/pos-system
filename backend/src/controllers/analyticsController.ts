...
function rangeConfig(range: string | undefined): RangeConfig {
  const now = new Date();
  if (range === "weekly") {
...
const popularFoods = asyncHandler(async (req: Request, res: Response) => {
...
const statusBreakdown = asyncHandler(async (req: Request, res: Response) => {
  const rows = await Order.aggregate([
...
const unavailableFoods = asyncHandler(async (req: Request, res: Response) => {
...
export { stats, sales, popularFoods, statusBreakdown, unavailableFoods, summary };
