...
import { ArrowRight, Clock, Leaf, Quote, ShieldCheck, Sparkles, Truck } from "lucide-react";
  ...
  return (
    <>
      <PageMeta
        ...
        description="Order from Meridian: dry-aged burgers, wood-fired pizza, free-range chicken and pastry-counter desserts, delivered warm across the city."
        ogDescription="Dry-aged burgers, wood-fired pizza and bar-grade drinks, delivered warm."
      />
      {/* Hero */}
      <section className="relative overflow-hidden border-b border-border">
        <div className="container-page grid items-center gap-12 py-16 lg:grid-cols-[1.05fr_1fr] lg:py-24">
          <div className="min-w-0">
            ...
            >
              Dry-aged beef, 72-hour dough and produce picked the same morning. Meridian runs one
              kitchen, one standard, and a delivery window we actually keep.
            </motion.p>
            <motion.div
              initial={reduced ? { opacity: 1 } : { opacity: 0, y: 14 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <Button asChild size="lg" variant="outline" className="rounded-full px-7">
                <Link
                  to={{ pathname: "/menu", search: buildMenuSearch({ q: "", category: "All" }) }}
                >
                  Explore menu
                </Link>
              </Button>
          ...
          >
            ...
            <div className="absolute -bottom-6 left-6 right-6 rounded-xl border border-border bg-card/95 p-4 backdrop-blur sm:left-auto sm:right-8 sm:w-64">
              ...
              <p className="mt-1 font-display text-xl text-foreground">Signature Burger</p>
      ...
      <section className="container-page py-20 pt-24">
        <Reveal className="mb-10 flex flex-wrap items-end justify-between gap-4">
          ...
          <Link to="/menu" className="text-sm text-primary underline-offset-4 hover:underline">
            View the full menu
          </Link>
        </Reveal>
              ...
              <Reveal key={review.id} delay={index * 0.06}>
                <figure className="flex h-full flex-col rounded-xl border border-border bg-card p-7">
                  ...
                  <figcaption className="mt-6 border-t border-border pt-5">
                    <span className="block text-sm font-medium text-foreground">{review.name}</span>
      ...
      <section className="border-t border-border bg-surface py-20">
        <div className="container-page grid gap-8 md:grid-cols-2 md:items-center">
          <Reveal>
            ...
            <h2 className="display-lg mt-3 text-foreground">Seasonal menus, first</h2>
            <p className="mt-4 max-w-md text-sm leading-relaxed text-muted-foreground">
              One email a month: new dishes, chef collaborations and the odd invitation to a tasting
              night. No noise.
            </p>
          </Reveal>
          <Reveal delay={0.08}>
